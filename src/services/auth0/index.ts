...
class Auth0Service {
  public generateAuth0Application = async (
    ...
    const donEnv = `${appPath}/.env`
    const donEnvExample = `${appPath}/.env.example`

    if (!fs.pathExistsSync(donEnvExample)) {
      throw new Error(`Unable to locate file .env.example in provided directory: ${appPath}`)
    }

    fs.copyFile(donEnvExample, donEnv, (error) => {
    ...
    fs.readFile(donEnv, 'utf-8', function (err, contents) {
      ...
      fs.writeFile(donEnv, updatedIssuer, 'utf-8')
    })
