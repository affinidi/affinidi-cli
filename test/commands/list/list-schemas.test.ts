...
describe('list schemas command', () => {
  ...
  describe('--view json', () => {
    ...
    test
      ...
      .nock(ANALYTICS_URL, (api) => api.post('/api/events').reply(StatusCodes.CREATED))
      .stdout()
      .command(['list schemas', '-v', 'json'])
      .it('runs list schemas -v json and shows schemas in json format', (ctx) => {
    ...
    test
      ...
      .nock(ANALYTICS_URL, (api) => api.post('/api/events').reply(StatusCodes.CREATED))
      .stdout()
      .command(['list schemas', '--view', 'json'])
      .it('runs list schemas --view json and shows schemas in json format', (ctx) => {
        expect(() => JSON.parse(ctx.stdout)).not.to.throw()
      })
  })
  ...
  describe('--view table', () => {
    ...
    test
      ...
      .nock(ANALYTICS_URL, (api) => api.post('/api/events').reply(StatusCodes.CREATED))
      .stdout()
      .command(['list schemas', '-v', 'table'])
      .it('runs list schemas -v table and shows a table with different schemas', (ctx) => {
    ...
    test
      ...
      .nock(ANALYTICS_URL, (api) => api.post('/api/events').reply(StatusCodes.CREATED))
      .stdout()
      .command(['list schemas', '--view', 'table'])
      .it('runs list schemas --view table and shows a table with different schemas', (ctx) => {
        expect(() => JSON.parse(ctx.stdout)).to.throw()
        tableHeaders.map((h) => expect(ctx.stdout).to.contain(h))
      })
  })
